...
function buildReplacements( { pluginName, slug, description, author, authorUri, pluginUri, nsVendor, nsPackage } ) {
	const fnPrefix      = toSnake( slug );
	const constPrefix   = toConstant( slug );
...
async function generate( initialName = '' ) {
	...
	try {
		...
		const authorEmail = await ask( rl, 'Author Email', gitEmail || '' );
		const authorUri   = await ask( rl, 'Author URI', 'https://yoursite.com' );
		const pluginUri   = await ask( rl, 'Plugin URI', `${ authorUri }/${ slug }` );
		const nsVendor    = await ask( rl, 'Namespace Vendor', toPascalCase( author.split( ' ' )[ 0 ] ) || 'MyPlugin' );
		...
		const context = { pluginName, slug, description, author, authorUri, pluginUri, nsVendor, nsPackage };
